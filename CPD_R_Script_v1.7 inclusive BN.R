...
# Log 6; 22/10/2018: Added the code to obtain the snr_dn and mac addresses time series datasets.
# Log 7; 22/10/2018: BNs can be produced as outputs.
# Log 8; xx/10/2018: 
# Log 9; xx/10/2018: 
...
# Create dataframe with node name, hour stamp and average SNR UP
...
# SNR Averages dataset query alternative;
...
# Obtain time series of a mac address
...
# --------Code for time series plot (used for presentation)-----------------------------------
...
# Report the dimensions of the datasets.
dim(testCasePolling)
dim(referenceTest)
dim(pollingTest)

# Report the structure of the datasets.
str(testCasePolling)
str(referenceTest)
str(pollingTest)
...
# Libraries for graphics (The last two are for plotting networks with probability bars)
#library(ggplot2)
#library(graphics)
#library(graph)
#library(igraph)
...
SNR_DN_dataframe <-
MAC_ADDRESS_dataframe <-

# 4.1 DATA CLEANING
#Factorize the dataframes
SNR_DN_dataframe$hour_stamp %>% as.factor(.)
SNR_DN_dataframe %>% mutate_if(is.integer, funs(factor(.)))
SNR_DN_dataframe %>% mutate_if(is.character, funs(factor(.)))

MAC_ADDRESS_dataframe$hour_stamp %>% as.factor(.)
MAC_ADDRESS_dataframe %>% mutate_if(is.integer, funs(factor(.)))
MAC_ADDRESS_dataframe %>% mutate_if(is.character, funs(factor(.)))

# To convert the integer values to factors
SNR_DN_dataframe[] <- lapply(BN_dataframe, as.factor)
MAC_ADDRESS_dataframe[] <- lapply(BN_dataframe, as.factor)

# to observe the levels of the variables
sapply(SNR_DN_dataframe, nlevels)
sapply(MAC_ADDRESS_dataframe, nlevels)

# 4.2 OBTAINING THE NETWORKS FROM THE EMBEDDED ALGORITHMS
...
Network_gs_SNR_DN = gs(SNR_DN_dataframe)
Network_gs_MAC_ADDRESS = gs(MAC_ADDRESS_dataframe)

# 4.2.2. Hill climb (a.k.a Greedy search) algorithm
Network_hc_SNR_DN = hc(SNR_DN_dataframe)
Network_hc_MAC_ADDRESS = hc(MAC_ADDRESS_dataframe)

# 4.2.3. Fast incremental association algorithm
Network_iamb_SNR_DN = iamb(SNR_DN_dataframe)
Network_iamb_MAC_ADDRESS = iamb(MAC_ADDRESS_dataframe)

# 4.2.4 Tabu network
Network_tabu_SNR_DN = tabu(SNR_DN_dataframe)
Network_tabu_MAC_ADDRESS = tabu(MAC_ADDRESS_dataframe)

# 4.3 BASIC PLOTTING OF THE BNs
# 4.3.1. Plotting the Grow-shrink algorithm network
plot(Network_gs_SNR_DN)
plot(Network_gs_MAC_ADDRESS)

# 4.3.2. plotting the Hill-climb algorithm network
plot(Network_hc_SNR_DN)
plot(Network_hc_MAC_ADDRESS)

# 4.3.3. plotting the IAMB algorithm network
plot(Network_iamb_SNR_DN)
plot(Network_iamb_MAC_ADDRESS)

# 4.3.4. plotting the Tabu algorithm results
plot(Network_tabu_SNR_DN)
plot(Network_tabu_MAC_ADDRESS)

# 4.4 PRINTING GRAPHS TO A PDF
pdf("Network Graphs.pdf")

# 4.4.1 Draw the network derived from grow shrink algorithm
graphviz.plot(Network_gs_SNR_DN, highlight = NULL, layout = "dot", shape = "circle", main = "Grow-Shrink Algorithm Network, SNR_DN Time Series Network", sub = NULL)
graphviz.plot(Network_gs_MAC_ADDRESS, highlight = NULL, layout = "dot", shape = "circle", main = "Grow-Shrink Algorithm Network, Mac Addresses Time Series Network", sub = NULL)

#4.4.2 Draw the network derived from hill climb algorithm
graphviz.plot(Network_hc_SNR_DN, highlight = NULL, layout = "dot", shape = "circle", main = "Hill-Climb Algorithm Network, SNR_DN Time Series Network", sub = NULL)
graphviz.plot(Network_hc_MAC_ADDRESS, highlight = NULL, layout = "dot", shape = "circle", main = "Hill-Climb Algorithm Network, Mac Addresses Time Series Network", sub = NULL)

# 4.4.3 Draw the network derived from iamb algorithm
graphviz.plot(Network_iamb_SNR_DN, highlight = NULL, layout = "dot", shape = "circle", main = "IAMB Algorithm Network, SNR_DN Time Series Network", sub = NULL)
graphviz.plot(Network_iamb_MAC_ADDRESS, highlight = NULL, layout = "dot", shape = "circle", main = "IAMB Algorithm Network, Mac Addresses Time Series Network", sub = NULL)

#4.4.4 Draw the network derived from tabu algorithm
graphviz.plot(Network_tabu_SNR_DN, highlight = NULL, layout = "dot", shape = "circle", main = "Tabu Algorithm Network, SNR_DN Time Series Network", sub = NULL)
graphviz.plot(Network_tabu_MAC_ADDRESS, highlight = NULL, layout = "dot", shape = "circle", main = "Tabu Algorithm Network, Mac Addresses Time Series Network", sub = NULL)
